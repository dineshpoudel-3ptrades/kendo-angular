...
import type { GetFeedbacksInput, FeedbackDto } from '../../../proxy/feedbacks/models';
import { FeedbackService } from '../../../proxy/feedbacks/feedback.service';

export abstract class AbstractFeedbackViewService {
  protected readonly proxyService = inject(FeedbackService);
  ...
  data: PagedResultDto<FeedbackDto> = {
    items: [],
    totalCount: 0,
  };

  selectionType = SelectionType;
  selected = signal<FeedbackDto[]>([]);
  allSelected = signal(false);
  selectedCount = computed(() => this.selected().length);

  filters = {} as GetFeedbacksInput;
  ...
  hookToQuery() {
    ...
    const setData = (list: PagedResultDto<FeedbackDto>) => {
      this.data = list;
  ...
  clearFilters() {
    this.filters = {} as GetFeedbacksInput;
    this.list.get();
  }
